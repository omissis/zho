...
function create {
  ...
  ctlptl apply -f "${CONFIG_DIR}/ctlptl-registry.yaml"
  KIND_IMAGE="kindest/node:v${CLUSTER_VERSION}" yq e '.kindV1Alpha4Cluster.nodes[0].image |= strenv(KIND_IMAGE)' < "${CONFIG_DIR}/kind-config.yaml" | ctlptl apply -f - || true
