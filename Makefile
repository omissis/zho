...
# TODO: replace this image with a zho-specific one
...
.PHONY: add-asdf-plugins update-asdf-tools
add-asdf-plugins:
	@./scripts/add-asdf-plugins.sh

update-asdf-tools:
	@./scripts/update-asdf-tools.sh
...
deploy-helm: check-variable-IMAGE_TAG_NAME check-variable-CLUSTER_NAME check-variable-NAMESPACE
	@helm upgrade -i ${PROJECT_NAME} helm_chart -f configs/helm-values/zho.yaml \
	 --namespace ${NAMESPACE} \
